...
def test_log_end_clamps_boundary_scores(capsys) -> None:
    log_end(True, 1, 0.0, [0.0])
    log_end(True, 1, 1.0, [0.0])

    out = capsys.readouterr().out.strip().splitlines()
    assert out[0] == "[END] success=true steps=1 score=0.001 rewards=0.00"
    assert out[1] == "[END] success=true steps=1 score=0.999 rewards=0.00"
